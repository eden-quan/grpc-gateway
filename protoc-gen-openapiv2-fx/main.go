...
import (
	...
	"gitlab.lainuoniao.cn/rhinobird/backend/grpc-gateway.git/protoc-gen-openapiv2-fx/internal/flatten"
...
func main() {
	...
	//os.Stdin = tempFile
	//
	// process stdin for reuse
	bufBytes, _ := io.ReadAll(os.Stdin)
	// 1. create temp file for original process
	os.Stdin = createTempFile(bufBytes)
	...
	// aster parse request
	// 2. create temp file for opts.parse
	os.Stdin = createTempFile(bufBytes)
	...
	// TODO: process the flatten message and fields
...
func parseReqParam(param string, f *flag.FlagSet, pkgMap map[string]string) error {
	...
	for _, p := range strings.Split(param, ",") {
		...
		if err := f.Set(name, value); err != nil {
			//return fmt.Errorf("cannot set flag %s: %w", p, err)
			fmt.Printf("cannot set flag %s: %w", p, err)
		}
	}
	return nil
}

func createTempFile(buf []byte) *os.File {
	f, _ := os.CreateTemp("", "temp_proto")
	_, _ = f.Write(buf)
	_, _ = f.Seek(0, 0)
	return f
}

func checkFlatten(req *pluginpb.CodeGeneratorRequest) {
	for _, f := range req.ProtoFile {
		for _, m := range f.MessageType {
			for _, f := range m.Field {
				flat := proto.GetExtension(f.Options, meta.E_Flatten).(bool)
				ext := proto.GetExtension(f.Options, meta.E_FlattenRule).(*meta.FlattenRules)
				if !flat && ext == nil {
					continue
				}
			}
